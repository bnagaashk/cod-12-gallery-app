...
class Gallery extends Component {
  ...
  changeback = userclickId => {
    this.setState({activeId: userclickId})
  }

  getfilteredData = () => {
    const {activeId} = this.state

    const filderedImagesList = imagesList.filter(
      eachImage => eachImage.id === activeId,
    )

    return filderedImagesList
  }

  render() {
    const filteredDatalist = this.getfilteredData()
    return (
      <div className="bg-container">
        <div className="card-container">
          <div>
            {filteredDatalist.map(eachImage => (
              <img
